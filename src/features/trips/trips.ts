...
export const countBy = (g: TripGroup) => TRIPS.filter((r) => r.group === g).length;
...
export interface RouteSummary {
  ...
  revenue: number; // combined across every trip on the route
  active: TripRow | null; // the trip currently tracked (first live one)
  nextDeparture: string | null; // next scheduled departure time
}

// Group trips into direction-specific routes; combine revenue; surface the active (tracked) trip.
export function deriveRoutes(trips: TripRow[]): RouteSummary[] {
  ...
  return [...map.entries()]
    .map(([key, list]) => {
      const first = list[0]!;
      const active = list.find((t) => t.group === 'active') ?? null;
      const next = list
        .filter((t) => t.group === 'scheduled')
        .sort((a, b) => a.departs.localeCompare(b.departs))[0];
      return {
        key,
        from: first.from,
        to: first.to,
        trips: [...list].sort((a, b) => a.departs.localeCompare(b.departs)),
        liveCount: list.filter((t) => t.group === 'active').length,
        scheduledCount: list.filter((t) => t.group === 'scheduled').length,
        completedCount: list.filter((t) => t.group === 'completed').length,
        revenue: list.reduce((s, t) => s + t.revenue, 0),
        active,
        nextDeparture: next ? next.departs : null,
      };
    })
    .sort((a, b) => b.liveCount - a.liveCount || b.trips.length - a.trips.length);
}
