...
import { useUsers, useAgents, useDrivers, type ApiUser } from '@/lib/api/hooks';
import { useSession } from '@/lib/auth/session';
import { ADMIN_ROLES } from '@/app/shell/nav';
import { InviteUserModal, EditUserModal } from './StaffModals';
...
// Staff (governance: managers/admins/agents) + Drivers (moved from Fleet — a people concern, not a
// vehicle concern) + Roster (day-by-day trips assigned to drivers), one place for people-management.
export function StaffPage() {
  const { t } = useTranslation();
  const sessionUser = useSession((s) => s.user);
  const isAdmin = ADMIN_ROLES.includes(sessionUser?.role ?? '');
  const [tab, setTab] = useState<'staff' | 'drivers' | 'roster'>(isAdmin ? 'staff' : 'drivers');
  ...
  const cards = [
    ...
    { label: t('staff.kpiDrivers'), value: drivers.length },
  ];

  return (
    <Reveal className="space-y-6">
      {isAdmin && <InviteUserModal open={invite} onClose={() => setInvite(false)} />}
      ...
      <RevealItem>
        <SectionTabs
          ariaLabel={t('nav.staff')}
          active={tab}
          onChange={setTab}
          tabs={[
            ...(isAdmin ? [{ key: 'staff' as const, label: t('team.tabs.staff') }] : []),
            { key: 'drivers', label: t('staff.tabs.drivers') },
            { key: 'roster', label: t('staff.tabs.roster') },
          ]}
        />
      </RevealItem>
      ...
      <RevealItem>
        {tab === 'staff' && isAdmin ? (
          <StaffPanel query={usersQ} onInvite={() => setInvite(true)} />
        ) : tab === 'roster' ? (
          <RosterPanel />
        ) : (
          <DriversPanel />
...
// returns agents by role, so staff + agents live in one list. Admin-only (see ADMIN_ROLES in nav.ts).
